...
- **Procedure skills** (invoked with `/name`; `sync-*`, `fresh-start`, `profile-create` and `create-sample` are user-only; `git-*` run automatically as part of every task): `/create-effect`, `/fresh-start`, `/git-commit`, `/git-create-branch`, `/git-rebase-main`, `/git-squash-commits`, `/git-submit-pr`, `/profile-create`, `/create-sample`, `/hardware-validation`, `/verify-implementation`, `/sync-components`, `/sync-template`.
