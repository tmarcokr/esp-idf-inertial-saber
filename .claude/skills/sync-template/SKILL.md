---
name: sync-template
description: Add or update the ESP-IDF template (.claude, .vscode, and CLAUDE.md) from the upstream repository. Use only when the user explicitly invokes /sync-template.
...
> **⚠️ Migration note:** The upstream `esp-idf-template` repository may still use the legacy Google Antigravity layout (`.agents/`, `GEMINI.md`). Until upstream is migrated to the Claude Code layout (`.claude/`, `CLAUDE.md`), this sync will not bring in matching paths. After fetching, check `git ls-tree -r --name-only template/main` and stop to ask the user if upstream still uses `.agents/`/`GEMINI.md`.

## Important Constraints
- **Do not use `git checkout`** for bringing files from the template (e.g., `git checkout template/main -- .claude`), as it will forcefully overwrite local changes without warning.
...
- Focus updates specifically on the `.claude/`, `.vscode/` directories, and the root `CLAUDE.md`.
...
3. **Merging Changes (Safe approach for specific folders)**:
   - To merge changes safely while only affecting `.claude/`, `.vscode/`, and `CLAUDE.md`, execute a merge without committing automatically:
     ```bash
...
4. **Filtering and Atomic Cleanup**:
   - Because a merge of unrelated histories downloads the entire template tree, you must isolate the desired changes and purge the rest:
     ...
     git add .claude/ .vscode/ CLAUDE.md
...
5. **Finalizing**:
   ...
   - If `CLAUDE.md` resulted in a conflict during merge, ensure you have combined the template's foundational rules with your project-specific context.
   - If everything looks correct, commit the merge:
     ```bash
     git commit -m "chore: sync .claude, .vscode and CLAUDE.md from esp-idf-template"
     ```
