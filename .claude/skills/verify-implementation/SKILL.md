...
description: Audit process to validate the technical quality of the code. Use every time a significant implementation is completed, before marking code as ready to flash, or when the user invokes /verify-implementation.
...
1. **Invoke the Auditor**:
   ...
   - Component to audit (if provided): `$ARGUMENTS`
   - The agent will activate the `quality-auditor` skill (or delegate to the `code-auditor` subagent) and verify against the rules in `.claude/rules/01-project-instructions.md`.
