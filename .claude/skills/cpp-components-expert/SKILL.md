...
- **Inspect Before Acting**: Always use the `Grep` or `Read` tools to inspect the actual `.hpp` and `.cpp` files in `components/` to understand the current interfaces, templates, and class structures.
